...
class Agent():
    ...
    async def get_completion_gen(self, message: str, run, threadname='main', color_output=True) -> AsyncGenerator[str, None]:
        """
    ...
    async def send_message_gen(self, message: str, threadname='main') -> AsyncGenerator[str, None]:
        """
        ...
        Yields:
            The response from the OpenAI API or the yielded values from get_completion_gen.
        """
        ...
        # Iterate through the responses from get_completion_gen
        print("getting messages")
        async for m in self.get_completion_gen(message, run_creation_response, threadname):
            # print('time')
            if isinstance(m, tuple):
                logging.debug(f"Yielding tuple response from get_completion_gen in thread '{threadname}'")
                yield m[0]  # Yield the first element of the tuple
            else:
                logging.debug(f"Yielding response from get_completion_gen in thread '{threadname}'")
                yield m
