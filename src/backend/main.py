...
async def upload_agent_data(new_data: AgentData):
    try:
        # Load existing data
        try:
            with open("JsonFiles/agent_lib.json", "r") as f:
        ...
        with open("JsonFiles/agent_lib.json", "w") as f:
...
async def upload_swarm_data(graphdata:GraphData):
    with open("JsonFiles/swarm_data.json", "w") as f:
            f.write(graphdata.model_dump_json())
...
async def stream():
    with open('JsonFiles/swarm_data.json', 'r') as f:
        ...
        schema, u = create_communication_schema(test)
        return StreamingResponse(schema.start(starting_node=u))