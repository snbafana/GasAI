...
from Actors.UtilityAgents import Splitter
from .Node import Node
from .Chats import ChatOne
import itertools
import logging
import asyncio
...
class Schema:
    ...
    def create_spliter(self, out:list[Node]) -> ChatOne:
        """
        ...
        splitter = Splitter(name=f"Splitter for {out}", descriptions=descriptions, out_nodes=out, comm=self)
        
        # Create the splitting node
        splitter_chat:ChatOne = ChatOne(actor=splitter, comm=outerself)
    ...
    def enable_auto_splitting(self):
        """
        Enables the auto-splitting feature in the network schema.

        When auto-splitting is enabled, the system automatically splits a user's query into
        multiple goals and processes them simultaneously across different nodes.
        """
        self.autosplitting = True
    ...
    async def singular_system_pass(self, message, starting_node:Node, threadname='main'):
        """
        ...
        while connected_nodes != []:
            ...
            if connected_nodes != []:
                node =  connected_nodes[0]
                if len(nx.ancestors(self.graph, node)) > 1:
                    yield node
                    return
